...
func retrieve_wave_data():
	...
	var wave_data = GameData.tower_data["Waves"][1]
	current_wave += 1
...
func initiate_build_mode(tower_type):
	if build_mode:
		cancel_build_mode()
	built_type = tower_type + "T1"
	build_mode = true
	get_node("UI").set_tower_preview(built_type, get_global_mouse_position())
...
func verify_and_build():
	if build_valid:
		var new_tower = load("res://Scenes/Turrets/" + built_type + ".tscn").instance()
		new_tower.position = build_location
		new_tower.built = true
		new_tower.type = built_type
		new_tower.category = GameData.tower_data[built_type]["category"]
		if new_tower.type == "MissileT1":
			new_tower.spawn_rockets()
